...
echo "--------------------------------"
echo "Email is: $1"
echo "www-domain is: $2"
echo "no-www-domain is: $3"
echo "..."
...
crontab -l | grep -q 'certbot'  && echo 'Auto Renewal is already in place' || (crontab -l 2>/dev/null; echo '15 3 * * * /usr/bin/certbot renew --quiet --deploy-hook "sudo systemctl reload nginx"') | crontab -
